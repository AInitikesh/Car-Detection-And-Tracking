...
After making the whole pipline I tested my solution on test video and project video. And I found that there is an issue with false positives. To reduce this effect I used a technique to save the identified windows of previous n frames and then add it to the current heat of image. This allowed me to increase the thresold of heat and make correct guesses. Even after applying this technique I was not able to get the results where there is tree shadow so I marked those portions and croped those frames to add it to training set. This helped me to remove the false positives. 
This pipline will fail where the is no enough light or if there is a vehicle other than another car. So my steps would be to collect such dataset and add it to training set to make it more robust. Also in future I would try deeplearning techniques like YOLO to overcome the limitations of this technique.
